...
class AcyclicPart(val mg: MergeGraph, excludeSet: Set[NodeID], useExtPart : Boolean) extends LazyLogging {
  ...
  def partition(smallPartCutoff: Int = 20) {
    mg.outputMGNeighborsEval()      //PROFILE SPEEDTEST ONLY 081723
    if(!useExtPart){
      val toApply = Seq(
    ...
    print("edgecut in: ")
    print(mg.inNeigh.map{neighborSet => neighborSet.size}.sum)
    print("edgecut out: ")
    print(mg.outNeigh.map{neighborSet => neighborSet.size}.sum)
