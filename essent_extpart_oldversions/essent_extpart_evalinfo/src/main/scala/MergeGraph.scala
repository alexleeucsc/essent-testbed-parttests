  ...
  def readInGraph(graphFile : String){
    ...
    for(l <- lines){
      ...
      if (l contains "InNeigh"){
        val line1 = l.split(":")(1).split("<1>")
        val inNeighVals = line1.map{neighList => neighList.split("<2>").map(member => if(member==""){0}else{member.toInt})}
        print(inNeighVals.size)
        print(" vs: ")
        print(inNeigh.size)
        print("\n")
        (0 until inNeighVals.size) map {idx => inNeigh(idx).clear}
        (0 until inNeighVals.size) map {idx => inNeigh(idx) :+ inNeighVals(idx)}
      }
      if (l contains "OutNeigh"){
        val line1 = l.split(":")(1).split("<1>")
        val outNeighVals= line1.map{neighList => neighList.split("<2>").map(member => if(member==""){0}else{member.toInt})}
        (0 until outNeighVals.size) map {idx => outNeigh(idx).clear}
        (0 until outNeighVals.size) map {idx => outNeigh(idx) :+ outNeighVals(idx)}
      }
