...
class ActivityTracker(w: Writer, opt: OptFlags) {
  ...
  def emitSigTracker(stmt: Statement, indentLevel: Int, sg: StatementGraph): Unit = {
    stmt match {
      case mw: MemWrite =>
      case _ => {
        ...
        resultName match {
          case Some(name) => {
            // if (sg.stateElemNames.toSet.contains(name) | extIO.contains(name)){
            //   val cleanName = name.replace('.', '$')
            // }else{
            //   val cleanName = name
            // }
            val cleanNameOnly = name.replace('.', '$')
            val cleanName = if(sg.stateElemNames.toSet.contains(name) | extIO.contains(name)){name}else{name.replace('.', '$')}
            w.writeLines(indentLevel, s"$sigTrackName[${sigNameToID(name)}] += $cleanName != old::$cleanNameOnly ? 1 : 0;")
            if (opt.trackExts) {
              w.writeLines(indentLevel, s"$sigActName[${sigNameToID(name)}] = $cleanName != old::$cleanNameOnly;")
            ...
            w.writeLines(indentLevel, s"old::$cleanNameOnly = $cleanName;")
          }
          case None =>
        }
      }
