...
class Renamer {
  ...
  def populateFromSG(sg: StatementGraph, extIOMap: Map[String,Type]) {
    ...
    // nameToMeta.keys.map{name => print(name + "\t" + nameToMeta(name).decType + "\n")}
    fixEmitNames()
  }
  ...
  def fixEmitNames() {
    ...
    //DEBUG: 1: all meta decTypes should be: RegSet, ExtIO, or Local
    //2: all meta names that are outputted and correctly recognized in TestHarness.h should have decType==Local
    //3: all meta names that are NOT recognized in TestHarness.h should have decType==(ExtIO or Local)
