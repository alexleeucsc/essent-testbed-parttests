...
class AcyclicPart(val mg: MergeGraph, excludeSet: Set[NodeID], useExtPart : Boolean, clusterTargetDagP : Int) extends LazyLogging {
  ...
  def partition(smallPartCutoff: Int = 20) {
    ...
    var f = new FileWriter("092823_rocket20_dot",true)
    var p = new java.io.PrintWriter(f)
    mg.outputGNeighbors_modfile_toDot(p)
    ...
    //dump info for partition quality eval
      // var f = new FileWriter("092523_fastpart",true)
      // var p = new java.io.PrintWriter(f)
      // mg.outputGNeighbors_modfile(p)
      // f = new FileWriter("092523_fastpart",true)
      ...
      //100123 - DEBUG: use second partition func
      // mg.readInGraphFromDagP("/soe/alexlee/alexlee/essent-testbed-document/essent-testbed-parttests/essent_extpart_evalCheckCopy/092823_rocket20_dot.dot.partsfile.part_7700.seed_0.txt")
      // var f2 = new FileWriter("092923_checkPartition",true)
      // var p2 = new java.io.PrintWriter(f2)
      // mg.outputGNeighbors_modfile(p2)
      // f2 = new FileWriter("092923_checkPartition",true)
      // p2 = new java.io.PrintWriter(f2)
      // mg.outputMGMergeInfo_modfile(p)
    assert(checkPartioning())
  }
  def partition(smallPartCutoff: Int, excludedIDs : Set[Int], printStmt : Int) {
    val startTime = System.currentTimeMillis()
    var f = new FileWriter(mg.dotGraphDumpFile, true)
    var p = new java.io.PrintWriter(f)
    mg.outputGNeighbors_modfile_toDot(p)
    if(clusterTargetDagP == -1){
      print("you didnt specify a clusterTargetDagP!")
      sys.exit(111)
    }
    val command = "/mada/users/alexlee/alexlee/dagP/exe/rMLGP " +
      mg.dotGraphDumpFile + " " + clusterTargetDagP + 
      " --nbproc 1 --ratio 1.03 --write_parts 1 --seed 1000"
    print("dagP command:")
    print(command)
    print("\n")
    val process = Process(command).run()
    process.exitValue()
    mg.readInGraphFromDagP("", excludedIDs, printStmt, clusterTargetDagP)
    var f2 = new FileWriter("092523_fastpart", true)
    var p2 = new java.io.PrintWriter(f2)
    mg.outputGNeighbors_modfile(p2)
    f2 = new FileWriter("092523_fastpart", true)
    p2 = new java.io.PrintWriter(f2)
    mg.outputMGMergeInfo_modfile(p2)
    val stopTime1 = System.currentTimeMillis()
    print("\n total partition function took ")
    print(stopTime1 - startTime)

    mg.outputMGNeighbors_dagpMergeInfo()
    mg.outputMGMergeInfo_dagpMergeInfo()
...
object AcyclicPart {
  def apply(g: Graph, excludeSet: Set[NodeID] = Set(), useExtPart : Boolean = false, clusterTargetDagP : Int = -1) = {
    val ap = new AcyclicPart(MergeGraph(g), excludeSet, useExtPart, clusterTargetDagP)
      print("\n----------dotgraphdumpfile mergeGraph make-----\n")
    print(ap.mg.dotGraphDumpFile)
    print("\n\n")
    ap
  }
}
