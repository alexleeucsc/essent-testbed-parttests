...
class MakeCondPart(sg: StatementGraph, rn: Renamer, extIOtypes: Map[String, Type], useExtPart : Boolean, clusterTargetDagP : Int) extends LazyLogging {
  val cacheSuffix = "$old"
  ...
  def doOpt(smallPartCutoff: Int = 20) {
    ...
    print(sg.dotGraphDumpFile)
    print("\n\n")
    val ap = AcyclicPart(sg, excludedIDs.toSet, useExtPart, clusterTargetDagP)
    print("\n----------dotgraphdumpfile set after acyclicPart made?-----\n")
    print(ap.mg.dotGraphDumpFile)
    print("\n\n")
    ap.partition(smallPartCutoff, excludedIDs.filter{id => !(printStatementsClump contains id)}.toSet, printStatementsClump.head)
...
object MakeCondPart {
  def apply(ng: StatementGraph, rn: Renamer, extIOtypes: Map[String, Type], useExtPart : Boolean, clusterTargetDagP : Int) = {
    new MakeCondPart(ng, rn, extIOtypes, useExtPart, clusterTargetDagP)
  }
}
