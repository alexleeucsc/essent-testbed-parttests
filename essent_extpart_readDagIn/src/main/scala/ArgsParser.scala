...
case class OptFlags(
    ...
    dotGraphDumpFile: String = null,
    useExtPart: Boolean = false,
    clusterTargetDagP : Int = -1) {
...
class ArgsParser {
  val parser = new OptionParser[OptFlags]("essent") {
    ...
    opt[String]("dotGraphDump").required().unbounded().action( (x, c) =>
      c.copy(dotGraphDumpFile = x) ).text("file to dump dot graph info to")

    opt[Unit]("externalPartitioner").action( (_, c) => c.copy(
        useExtPart = true,
    )).text("use external partitioner")

    opt[Int]("clusterTargetDagP").action( (x, c) => c.copy(
        clusterTargetDagP = x)
    ).text("number of clusters to tell dagP to get")
