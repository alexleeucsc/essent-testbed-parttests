...
class EssentEmitter(initialOpt: OptFlags, w: Writer, circuit: Circuit) extends LazyLogging {
  ...
  def writeBodyInner(indentLevel: Int, sg: StatementGraph, opt: OptFlags,
    ...
    sg.stmtsOrdered foreach { stmt => stmt match {
      ...
      case _ => {
        ...
        if (opt.trackSigWithinMux) actTrac.emitSigTracker(stmt, indentLevel)
      }
    }}
  }

  def getSigCount(sg: StatementGraph, opt: OptFlags,
                     keepAvail: Set[String] = Set()) : Int = {
    sg.stmtsOrdered.map { stmt => stmt match {
      case cm: CondMux => {        
        getSigCount(StatementGraph(cm.fWay), opt) + getSigCount(StatementGraph(cm.tWay), opt)
      }
      case _ => {
        1
      }
  ...
  def writeZoningPredecs(
    ...
    w.writeLines(0, "")
    print("\n--------------------\n")
    print("sg.stmtsOrdered.size: " + sg.stmtsOrdered.size.toString)
    print("\n")
    print("TopologicalSort(sg).size: " + TopologicalSort(sg).size.toString)
    print("\n")
    print("validNodes.size: " + sg.validNodes.size.toString)
    print("\n")
    var evalIDToSize = HashMap[Int, Int]()
    var evalIDToEdgeCut = HashMap[Int, Int]()
    sg.stmtsOrdered foreach { stmt => stmt match {
      case cp: CondPart => {
        ...
        if (opt.printOutClustSizes){
          evalIDToSize(cp.id) = getSigCount(bodySG, opt, keepAvail)
        }
        val regUpdateNamesInPart = regUpdates flatMap findResultName
        val partFlagStrings = genAllTriggers(cp.outputsToDeclare.keys.toSeq, outputConsumers, condPartWorker.cacheSuffix)
        val regUpdateStrings = genAllTriggers(regUpdateNamesInPart, outputConsumers, "$next")
        if (opt.printOutClustCutCount){
          evalIDToEdgeCut(cp.id) = partFlagStrings.size
        }
        w.writeLines(2, partFlagStrings)
        w.writeLines(2, regUpdateStrings)
    ...
    if (opt.printOutClustSizes){
      val maxIdx: Int = evalIDToSize.keys.max
      val evalIDToSizeArr: Array[Int] = Array.fill(maxIdx + 1)(0)
      evalIDToSize.foreach { case (idx, size) =>
        evalIDToSizeArr(idx) = size
      }
      w.writeLines(32, "\nstd::vector<int> myVec{")
      val evalIDToSizeArr_str: String = evalIDToSizeArr.mkString(",")
      print("evalIDToSizeArr_str", evalIDToSizeArr_str)
      w.writeLines(2, evalIDToSizeArr_str)
      w.writeLines(2, "};\n")
      w.writeLines(0, "")
    }
    if (opt.printOutClustCutCount){
      val maxIdx: Int = evalIDToEdgeCut.keys.max
      val evalIDToEdgeCutArr: Array[Int] = Array.fill(maxIdx + 1)(0)
      evalIDToEdgeCut.foreach { case (idx, size) =>
        evalIDToEdgeCutArr(idx) = size
      }
      w.writeLines(32, "\nstd::vector<int> edgeCutVec{")
      val evalIDToEdgeCutArr_str: String = evalIDToEdgeCutArr.mkString(",")
      print("evalIDToEdgeCutArr_str", evalIDToEdgeCutArr_str)
      w.writeLines(2, evalIDToEdgeCutArr_str)
      w.writeLines(2, "};\n")
      w.writeLines(0, "")
    }
  }
  ...
  def execute(circuit: Circuit) {
    ...
    // print("validNodes.size: " + sg.validNodes.size.toString)
    // print("\n\n")
    sg.setGraphDumpFile(opt.graphDumpFile)
    logger.info(sg.makeStatsString)
    val containsAsserts = sg.containsStmtOfType[Stop]()
    val extIOMap = findExternalPorts(circuit)
    print("opt.useExtPart:",opt.useExtPart)
    val condPartWorker = MakeCondPart(sg, rn, extIOMap, opt.useExtPart, opt.dumpPreMergeInfo, opt.internalMergeInfo, opt.timeSpentCycleChecking, opt.origCpp, opt)
    ...
    // if(opt.trackSigWithinMux){
    //   w.writeLines(2, "SIGTotal=0;")
    // }
    w.writeLines(0, "")
