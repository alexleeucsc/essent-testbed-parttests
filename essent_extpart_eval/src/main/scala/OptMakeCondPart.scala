...
class MakeCondPart(sg: StatementGraph, rn: Renamer, extIOtypes: Map[String, Type], useExtPart : Boolean, dumpPreMergeInfo: Boolean, internalMergeInfo: Boolean, timeSpentCycleChecking: Boolean, origCpp: Boolean, opt: OptFlags) extends LazyLogging {
  val cacheSuffix = "$old"
  ...
  def doOpt(smallPartCutoff: Int = 20) {
    ...
    if(useExtPart || origCpp){
      sg.outputSGValidNodes() //you need this to run external partitioner
    }
      //activation evaluation
      // val f = new FileWriter("112023_fastpart",true)
      // val p = new java.io.PrintWriter(f)
      // sg.outputSGNames_modfile(p)
      // (to debug) replaced by premerge
    if(dumpPreMergeInfo){
      sg.outputSGValidNodesEval()
    }
    val ap = AcyclicPart(sg, excludedIDs.toSet, useExtPart, dumpPreMergeInfo, internalMergeInfo, timeSpentCycleChecking, origCpp, opt)
...
object MakeCondPart {
  def apply(ng: StatementGraph, rn: Renamer, extIOtypes: Map[String, Type], useExtPart : Boolean, dumpPreMergeInfo: Boolean, internalMergeInfo: Boolean, timeSpentCycleChecking: Boolean, origCpp: Boolean, opt: OptFlags) = {
    new MakeCondPart(ng, rn, extIOtypes, useExtPart, dumpPreMergeInfo,internalMergeInfo,timeSpentCycleChecking, origCpp, opt)
  }
}