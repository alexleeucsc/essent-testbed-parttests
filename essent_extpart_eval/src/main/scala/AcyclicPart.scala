...
class AcyclicPart(val mg: MergeGraph, excludeSet: Set[NodeID], useExtPart : Boolean, dumpPreMergeInfo: Boolean, internalMergeInfo: Boolean, timeSpentCycleChecking: Boolean, origCpp: Boolean, opt: OptFlags) extends LazyLogging {

	//042523 - stores mergeIDs of big clusters we don't want to keep merging	
  var clustersToKeep = Set[NodeID] ()
  var timeInCycleCheck: Long = 0
  ...
  def perfomMergesIfPossible(mergesToConsider: Seq[Seq[NodeID]]) = {
    val mergesMade = mergesToConsider flatMap { mergeReq => {
      ...
      if(partsStillExist){
        ...
        isAcyclic = mg.mergeIsAcyclic(mergeReq.toSet)
        var stopTime = System.currentTimeMillis()
        timeInCycleCheck = timeInCycleCheck + stopTime - startTime
      }
    ...
    print("\ttime spent in cycleChecking ")
    print(timeInCycleCheck)
    print("\n")
    mergesMade
  }
  ...
  def mergeSmallParts(smallPartCutoff: Int = 20, mergeThreshold: Double = 0.5) {
    ...
    val mergesToConsider = smallPartIDs flatMap { id => {
      ...
      val choices = \ filter { _._1 >= mergeThreshold }
  ...
  def partition(smallPartCutoff: Int = 20) {
    ...
    if(dumpPreMergeInfo){
      mg.outputMGNeighborsEval()
    }
    if((!useExtPart & !origCpp) & (! opt.readInGraphDirect) ){
      ...
      toApply foreach { case (label, func) => {
        ...
        print(s"[$label] took: ${stopTime - startTime}\n")
        print(s"Down to ${partsRemaining()} parts\n")
      }}
      if(internalMergeInfo){
        mg.outputMGNeighbors_internalMergeInfo()
        mg.outputMGMergeInfo_internalMergeInfo()
      }
      // var f = new FileWriter("112023_fastpart",true)
      // var p = new java.io.PrintWriter(f)
      // mg.outputGNeighbors_modfile(p)
      // f = new FileWriter("112023_fastpart",true)
      // p = new java.io.PrintWriter(f)
      // mg.outputMGMergeInfo_modfile(p)
    }
    else if ((useExtPart | origCpp) & (! opt.readInGraphDirect)){
      mg.outputMGNeighbors()
      var command = ""
      if (origCpp){
        command = "externalPart_2_cOrig/a.out " + smallPartCutoff.toString + " " + mg.graphDumpFile
      }else{
        command = "externalPart_2_topotest/a.out " + smallPartCutoff.toString + " " + mg.graphDumpFile
      }
      ...
      // p = new java.io.PrintWriter(f)
      // mg.outputMGMergeInfo_modfile(p)
    }else if (opt.readInGraphDirect){
      print("\nreadin direct\n")
      mg.readInGraph_fromArg(opt.readInGraphDirectPath)
    }
    val sumOfInnerArrayLengths: Int = mg.inNeigh.map(innerArray => innerArray.length).sum
    print("\n[total edgecut] ")
    print(sumOfInnerArrayLengths)
    print("\n")
    val stopTime1 = System.currentTimeMillis()
    print("\n total partition function took ")
    print(stopTime1 - startTime)
...
object AcyclicPart {
  def apply(g: Graph, excludeSet: Set[NodeID] = Set(), useExtPart : Boolean = false, dumpPreMergeInfo: Boolean = false, internalMergeInfo: Boolean = false, timeSpentCycleChecking: Boolean = false, origCpp: Boolean = false,  opt: OptFlags = OptFlags()) = {
    val ap = new AcyclicPart(MergeGraph(g), excludeSet, useExtPart, dumpPreMergeInfo, internalMergeInfo, timeSpentCycleChecking, origCpp, opt)
