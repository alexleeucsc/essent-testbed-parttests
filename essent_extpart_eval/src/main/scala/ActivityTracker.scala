...
class ActivityTracker(w: Writer, opt: OptFlags) {
  ...
  def declareSigTracking(sg: StatementGraph, topName: String): Unit = {
    ...
    if (opt.trackSigWithinMux){
      w.writeLines(0, s"long long int $sigTotalRealName = 0;")
    }
      ...
      case _ => {
        ...
        resultName match {
          case Some(name) => {
            ...
            w.writeLines(indentLevel, s"$sigTrackName[${sigNameToID(name)}] += $cleanName != old::$cleanName ? 1 : 0;")
            if (opt.trackExts) {
              w.writeLines(indentLevel, s"$sigActName[${sigNameToID(name)}] = $cleanName != old::$cleanName;")
              val depNames = findDependencesStmt(stmt).head.deps
              val trackedDepNames = depNames filter sigNameToID.contains
              val depTrackers = trackedDepNames map { cleanName => s"$sigActName[${sigNameToID(name)}]" }
            ...
            if (opt.trackSigWithinMux){
              w.writeLines(indentLevel, s"$sigTotalRealName += 1;")
            }
            w.writeLines(indentLevel, s"old::$cleanName = $cleanName;")
          }
          case None =>
        }
      }
