...
/**
 * Constructor
 * @param inNeigh, outNeigh
 */
//--------------------------------------------------------------------------------------------------
Graph::Graph(const std::vector<std::vector<unsigned>>& inNeigh, const std::vector<std::vector<unsigned>>& outNeigh) :
    n(0){
    nb = outNeigh;
    pd = inNeigh;
    deg.reserve(outNeigh.size());
    indeg.reserve(outNeigh.size());
    for(int i=0; i<outNeigh.size(); i++){
      deg.push_back(outNeigh[i].size());
      indeg.push_back(inNeigh[i].size());
      if(inNeigh[i].size()==0){
        roots.push_back(i);
      }
      if(outNeigh[i].size()==0){
        leaves.push_back(i);
      }
    }
    n = outNeigh.size();
