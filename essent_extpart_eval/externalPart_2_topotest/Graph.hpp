...
class graphInfo{
    ...
    bool mergeIsAcyclic(NodeID u, NodeID v) {
        int begin_mergeIsAcyclic = std::clock();
        bool outBool = !extPathExists(u, v) && !extPathExists(v, u);
        int end_mergeIsAcyclic = std::clock();
        totalTimeCycleChecking = totalTimeCycleChecking + (end_mergeIsAcyclic - begin_mergeIsAcyclic);
        return outBool;
    }
